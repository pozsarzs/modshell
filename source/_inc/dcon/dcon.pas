...
function dc_readwrite(protocol, device: word; txdata: string;
...
begin
  ...
  begin
    ...
    // checksum check
    if checksum then
      if not dc_chkchecksum(leftstr(tgm, length(tgm) - 2), rightstr(tgm, 2))
        then result := '__';
  end
    else
      {$IFNDEF X}
...
function dcon_readwrite(connection: integer; txdata: string;
         checksum: boolean): string;
begin
  result := '';
  // check validity
  ...
  if prot[conn[connection].prot].prottype = 3
    then result := dc_readwrite(conn[connection].prot, conn[connection].dev,
         txdata, checksum)
    else
      {$IFNDEF X}
        writeln(ERR56);
      {$ELSE}
        Form1.Memo1.Lines.Add(ERR56);
      {$ENDIF}
end;
